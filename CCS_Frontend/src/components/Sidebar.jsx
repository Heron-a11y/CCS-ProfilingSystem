  ...
  return (
    <div
      className={`fixed inset-y-0 left-0 z-50 flex flex-col h-full border-r transition-all duration-300 ease-in-out ${
        isVisible ? 'w-64 shadow-2xl shadow-slate-900/50' : 'w-16'
      } ${dark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Header */}
      <div className={`flex items-center h-20 overflow-hidden shrink-0 transition-all duration-300 border-b ${
        dark ? 'border-slate-800' : 'border-slate-100'
      } ${isVisible ? 'px-4 justify-between' : 'px-0 justify-center'}`}>
      ...
      {/* Nav items */}
      <div className="flex-1 overflow-y-auto px-3 py-6 space-y-1 overflow-x-hidden">
        <p className={`px-2 text-xs font-semibold uppercase tracking-wider mb-4 transition-all duration-300 whitespace-nowrap ${
          isVisible ? 'opacity-100' : 'opacity-0 h-0 hidden'
        ...
        {modules.map(module => {
          ...
          return (
            ...
            >
              ...
              <module.Icon className={`w-5 h-5 shrink-0 transition-colors duration-300 ${
                isVisible ? 'mr-3' : 'mr-0'
              } ${isActive
                ? 'text-brand-400'
              ...
              <span className={`font-medium tracking-wide whitespace-nowrap transition-all duration-300 ${
                isVisible ? 'opacity-100 w-auto inline-block' : 'opacity-0 w-0 hidden'
              }`}>
                {module.label}
              </span>
            </button>
      ...
      <div className={`p-3 border-t space-y-2 ${dark ? 'border-slate-800' : 'border-slate-100'}`}>
        <button
          ...
          className={`w-full flex items-center rounded-xl transition-all duration-300 group border border-transparent hover:border-red-500/20 ${
            isVisible ? 'px-4 py-3' : 'px-0 py-3 justify-center'
        ...
        <div className={`flex items-center rounded-xl border transition-all duration-300 ${
          isVisible ? 'p-3' : 'p-2 justify-center'
