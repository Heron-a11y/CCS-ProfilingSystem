...
const EditFacultyModal = ({ isOpen, onClose, onFacultyUpdated, faculty }) => {
        ...
        <div className={`relative transform overflow-hidden rounded-2xl text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-4xl border ${modalBg}`}>
          ...
          <div className={`px-6 pt-6 pb-5 border-b ${divider} ${dark ? 'bg-gradient-to-br from-slate-800 to-slate-900' : 'bg-gradient-to-br from-orange-50/60 to-white'}`}>
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className={`text-[10px] font-bold uppercase tracking-widest mb-0.5 ${dark ? 'text-orange-400' : 'text-orange-500'}`}>Editing Faculty</p>
                <h3 className={`text-lg font-extrabold leading-tight ${boldText}`}>
                  {formData.first_name || 'First'} {formData.last_name || 'Last'}
                </h3>
                <p className={`text-xs mt-0.5 ${subText}`}>{formData.position || 'Position'}</p>
              </div>
            </div>
          </div>
          ...
          <div className={`px-6 py-3 flex justify-end gap-3 border-t ${footerBg}`}>
            ...
            <button type="submit" form="edit-faculty-form" disabled={isSubmitting}
              onClick={handleSubmit}
              className="px-4 py-2 text-sm font-semibold rounded-xl bg-orange-500 hover:bg-orange-600 text-white transition-colors shadow-lg shadow-orange-500/30 disabled:opacity-50 flex items-center gap-2 min-w-[110px] justify-center">
              {isSubmitting
